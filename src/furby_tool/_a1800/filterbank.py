...
from .tables_generated import (
    COSINE_MOD_MATRIX,
    FWD_COSINE_MOD_MATRIX,
    FWD_FILTERBANK_COEFF_0,
    FWD_FILTERBANK_COEFF_1,
    FWD_FILTERBANK_COEFF_2,
    FWD_FILTERBANK_COEFF_3,
    FWD_FILTERBANK_COEFF_4,
    FILTERBANK_COEFF_0,
    FILTERBANK_COEFF_1,
    FILTERBANK_COEFF_2,
    FILTERBANK_COEFF_3,
...
def forward(input_samples: list[int], output: list[int], frame_size: int) -> None:
    n = frame_size
    buf_a = [0] * 320
    buf_b = [0] * 320

    fwd_butterfly_32(input_samples, buf_a, n, 0)
    fwd_butterfly_32(buf_a, buf_b, n, 1)
    fwd_butterfly_32(buf_b, buf_a, n, 2)
    fwd_butterfly_32(buf_a, buf_b, n, 3)
    fwd_butterfly_32(buf_b, buf_a, n, 4)
    ...
    fwd_reconstruct(buf_a, buf_b, n, 4, FWD_FILTERBANK_COEFF_0)
    fwd_reconstruct(buf_b, buf_a, n, 3, FWD_FILTERBANK_COEFF_1)
    fwd_reconstruct(buf_a, buf_b, n, 2, FWD_FILTERBANK_COEFF_2)
    fwd_reconstruct(buf_b, buf_a, n, 1, FWD_FILTERBANK_COEFF_3)
...
def fwd_butterfly_32(src: list[int], dst: list[int], n: int, stage: int) -> None:
    group_size = n >> stage
    num_groups = 1 << stage
...
def fwd_reconstruct(src: list[int], dst: list[int], n: int, stage: int, coeffs: list[int]) -> None:
    group_size = shr(n, stage)
    num_groups = shl(1, stage)
    half = group_size // 2
    ...
    for g in range(num_groups):
        ...
        while dst_front < dst_back:
            ...
            val_a = extract_h(l_mac(l_mac(0, c0, a), negate(c1), c))
            val_b = extract_h(l_mac(l_mac(0, c1, a), c0, c))
            val_c = extract_h(l_mac(l_mac(0, c2, b), c3, d))
            val_d = extract_h(l_mac(l_mac(0, c3, b), negate(c2), d))
